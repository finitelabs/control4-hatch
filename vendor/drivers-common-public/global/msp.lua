...
JSON = require("JSON")
-- Use the finite-labs JSON (classic JSON.lua). It asserts on parse errors where
-- Snap One's json returns nil, so suppress that to preserve msp.lua's expectations.
JSON.assert = function() end
...
-- Metrics telemetry stripped: finite-labs drivers do not ship Snap One metrics.
-- No-op Metrics / MetricsHandler so the instrumentation calls below are harmless.
local _metricsNoop = function() end
local _metricsMeta = {
  __index = function()
    return _metricsNoop
  end,
...
Metrics = {
  new = function()
    return setmetatable({}, _metricsMeta)
  end,
}
MetricsHandler = setmetatable({}, _metricsMeta)
...
function OnDriverLateInit(driverInitType)
  ...
  if PersistData.AuthSettings.password then
    local _update = function()
      local password = C4:Decrypt(
        "AES-256-ECB",
        C4:GetDriverConfigInfo("model"),
        nil,
        PersistData.AuthSettings.password,
        AES_DEC_DEFAULTS
      )
      if password then
...
function SkipFwd(roomId)
  ...
  if thisQ then
    if thisQ.RADIO then
      if thisQ.nextProgrammedTrackRequested then
        return
      end
      if thisQ.SKIPS >= MAX_SKIPS then
        return
      end
...
function GetDashboardByQueue(qId)
  ...
  if thisQ then
    ...
    if
      (
        (thisQ.RADIO and thisQ.SKIPS < MAX_SKIPS)
        or (thisQ.CurrentTrack and (thisQ.REPEAT or thisQ.CurrentTrack < #thisQ.Q))
      ) and not thisQ.STREAM
    then
  ...
  if thisQ then
    ...
    if thisQ.CurrentState == "PLAY" then
      ...
      local _timer = function(timer)
        ...
        if
          SUPPORTS_GAPLESS
          and thisQ.CurrentTrackElapsed + 15 >= thisQ.CurrentTrackDuration
          and thisQ.CurrentTrackDuration ~= 0
        then
...
function ParseQueueStreamStatus(status)
  ...
  for k, v in pairs(CopyTable(ret)) do
    ret[k] = string.gsub(v, "~!@(.-)@!~", function(a)
      return escapes[tonumber(a)]
    end)
...
function GetFutureProgrammedTrack(program, roomId)
  if program == nil or roomId == nil then
    return
  end
...
function GetFutureTrackURL(track, roomId)
  if track == nil or roomId == nil then
    return
  end
...
function SelectDefaultItemInRoom(roomId) end

function SelectMediaDBItemInRoom(itemId, roomId) end
...
function Navigator:LogInCommand(idBinding, seq, args)
  ...
  if not password and self.AuthSettings.password then
    password =
      C4:Decrypt("AES-256-CBC", C4:GetDriverConfigInfo("model"), nil, self.AuthSettings.password, AES_DEC_DEFAULTS)
  end
  if username and password then
    if type(Login) == "function" then
