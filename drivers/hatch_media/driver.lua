...
--- Select this media service as the audio source in the given rooms. This is
--- what establishes the Control4 listening session (now-playing + transport +
--- volume routed to our amplifier endpoint); without it the sound plays on the
--- Hatch but Navigator shows no session to stop or adjust volume on. deviceid
--- is our media-service proxy device (C4:GetProxyDevices()); Control4 resolves
--- the audio path from there through the amplifier endpoint into the room.
...
local function applyState(tParams)
  ...
  updateNowPlaying(title, state.isPlaying and "Hatch" or nil)
...
--- Display labels for the favorites list, disambiguating duplicate names.
--- The Hatch app lets a user give two favorites the same name (e.g. two "Brown
--- Noise" touch-ring favorites that differ only in volume/color), and the API
--- carries no other human-readable label -- just id, displayOrder, and step
--- content. When a name repeats, append an occurrence counter so both entries
--- are selectable and tell apart. Indexed to line up with the favorites array.
...
function OnDriverLateInit()
  ...
  -- Ask the coordinator to push our current catalog + state.
